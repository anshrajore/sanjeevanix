          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-black/20">
              <button
                ...
                title={wakeSupported ? 'Toggle "Hey Sanjeevani" wake-word' : "Wake-word unsupported in this browser"}
                className={`flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full border transition ${
                  wakeListening
                    ? "bg-[#34D399]/20 border-[#34D399]/40 text-[#34D399]"
                    : "bg-white/5 border-white/10 text-white/60 hover:text-white"
                } disabled:opacity-40 disabled:cursor-not-allowed`}
              >
                <Ear className={`w-3 h-3 ${wakeListening ? "animate-pulse" : ""}`} />
                {wakeListening ? "Listening for wake-word" : 'Say "Hey Sanjeevani"'}
              </button>
              <button
                onClick={toggleCall}
                className={`ml-auto flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full border transition ${
                  callActive
                    ? "bg-[#FF4D6D]/20 border-[#FF4D6D]/40 text-[#FF8A9A]"
                    : "bg-white/5 border-white/10 text-white/60 hover:text-white"
                }`}
              >
                {callActive ? <PhoneOff className="w-3 h-3" /> : <Phone className="w-3 h-3" />}
                {callActive ? "End call" : "Voice call"}
              </button>
            </div>
            {vapiError && (
              <div className="px-4 py-1.5 text-[10px] text-red-300 bg-red-500/10 border-b border-red-500/20">
                {vapiError}
              </div>
            )}
