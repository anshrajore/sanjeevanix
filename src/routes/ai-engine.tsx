...
export const Route = createFileRoute("/ai-engine")({
  ...
  component: () => (
    <SubPage
      ...
      subtitle="Run a simulated match. See how Sanjeevani scores donors by trust, distance, availability, and acceptance prediction."
    >
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="glass rounded-2xl p-8 space-y-4">
          ...
          <button className="w-full bg-gradient-to-r from-[#FF4D6D] to-[#E63946] text-white py-3 rounded-xl glow-red font-medium">
            Run Matching
          </button>
        </div>
        <div className="glass-red rounded-2xl p-8">
          ...
          {[
            ...
            <div key={d.n} className="flex items-center gap-3 bg-black/30 rounded-xl p-3 mb-2">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#FF4D6D] to-[#E63946] flex items-center justify-center font-bold text-sm">
                {d.n[0]}
              </div>
              <div className="flex-1">
                <div className="font-medium">{d.n}</div>
                <div className="text-xs text-white/50">
                  {d.d} · Trust {d.t}% · Accept {d.a}%
                </div>
              </div>
              <div className="font-display text-2xl text-gradient-red">
                {Math.round((d.t + d.a) / 2)}
              </div>
            </div>
          ))}
        </div>
